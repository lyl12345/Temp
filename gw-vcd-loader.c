...
{
    GwTreeNode* scope = gw_tree_builder_push_scope(self->tree_builder, kind, name);
    scope->t_which = -1;
}
...
void gw_vcd_loader_var(GwVcdLoader* self, unsigned int type, int size, const gchar* name)
{
    ...
    v->name = gw_tree_builder_get_symbol_name(self->tree_builder, name);
...
{
    vcd_parse_enddefinitions(self, NULL);
}

void gw_vcd_add_time(GwVcdLoader* self, GwTime tim)
{
    if (!self->header_over) {
        self->header_over = TRUE; /* do symbol table management here */
        create_sorted_table(self);
...
void gw_vcd_add_signal_action(GwVcdLoader* self, const gchar* name, char action)
{
    ...
    if (1) {
        v = bsearch_vcd(self, name, strlen(name));
        if (!v) {
            fprintf(stderr,
                "Near byte %d, Unknown VCD identifier: '%s'\n",
                (int)(self->vcdbyteno + (self->vst - self->vcdbuf)),
...
GwDumpFile* gw_vcd_loader_end(GwVcdLoader* self)
{
    ...
    dump_file->preserve_glitches = gw_loader_is_preserve_glitches(self);
    dump_file->preserve_glitches_real = gw_loader_is_preserve_glitches_real(self);
